...
enh_allgenes %>% filter(!is.na(ENHANCERID)) %>% 
  dplyr::select(Time,ENHANCERID,TF,Reads,GR_lou_WT,GR_lou_dex) %>%
  unique %>% 
  pivot_wider(names_from = TF,values_from = Reads) -> data_master
...
ll_enh %>% as_tibble %>%mutate(GR=(GR_lou_dex==TRUE)|(GR_lou_WT==TRUE)) %>% dplyr::select(ENHANCERID,mean_bc,GR) %>% unique %>% as_tibble %>%
...
ggsave(plot = p_enhancers,filename = "~/GRanalysis_master/GR_CircadianLiverTranscriptome/Fig2/Plots/dnase_mult_enhancers_rev.png",width=4,height=3)
...
ll_enh %>% mutate(GR=(GR_lou_dex==TRUE)|(GR_lou_WT==TRUE)) %>% 
  group_by(ENHANCERID) %>%
  ...
  mutate(REVERBa=mean(`REVERBa*`,na.rm=T)) %>%  #need reverb
  dplyr::select(ENHANCERID,bc=mean_bc,GR,BMAL1,CLOCK,CRY1,CRY2,NPAS2,PER1,PER2,REVERBa) %>% unique %>% as_tibble -> mod_data
...
lm(REVERBa ~ bc + GR,data = mod_data) -> model_1
standardize(model_1, method = "refit")  # standardizes predictors and outcome
effectsize::standardize_parameters(model_1, method = "refit")
