...
rbind(data_master_all %>% dplyr::select(GeneID,ENHANCERID,PROMID.x,GRlou_WT_us,GRlou_dex_us,JTK_pvalue=JTK_pvalue.x,JTK_adjphase=JTK_adjphase.x,RNA,RNA1:RNA24),
      data_master_all_2 %>% dplyr::select(GeneID=To,ENHANCERID,PROMID.x=PROMID,GRlou_WT_us,GRlou_dex_us,JTK_pvalue=JTK_pvalue.x,JTK_adjphase=JTK_adjphase.x,RNA,RNA1:RNA24)) -> data_master_chicandnearest

(data_master_chicandnearest %>% 
...
ggsave("Fig2/Plots/p_phase_p_both.png",p_phase_p,height =4,width = 6.5)
...
venn.diagram(list(`GR Bound` = which(venn_data$Both_us.g),Rhythmic = which(venn_data$Rhythmic.g)), 
             ...
             lwd =1,
             cat.cex=0.6,
             height=1500,width=1500,  rotation.degree = 0,
             reverse = FALSE,"Fig2/Plots/venn_diagram_both.png")
...
ggsave(RNA_plot,filename = "Fig2/Plots/rna_boxplots_fitz_both.png",height = 3,width = 4)
...
data_master_chicandnearest %>% 
  mutate(RNA_m=data_master_chicandnearest %>% dplyr::select(RNA1:RNA24) %>% apply(1,mean)) %>%
...
ggsave(filename = "Fig2/Plots/supp_quants_both.png",width=5,height=4)
...
ggsave(filename = "Fig2/Plots/suppB_both.png",p,width=6,height=4)
