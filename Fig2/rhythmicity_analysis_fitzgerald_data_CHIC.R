...
chic<-read.csv("~/GRanalysis_master/GR_CircadianLiverTranscriptome/Data/All_step2_washU_text_symm.txt", sep = "\t", header = F)
...
for(i in chic_filt[,7]){
  which(chic_bait_bed$X==i)
  which(chic_otherends_bed$X==i)
  TSS_ourproms_Granges[from(prom_bait_overlaps[to(prom_bait_overlaps)==i]),] %>% as.data.frame() -> temp_proms
  names(temp_proms) <- temp_proms %>% names %>% paste0("prom_",.)
  ourenhs_Granges[from(enh_otherend_overlaps[to(enh_otherend_overlaps)==i]),] %>% as.data.frame() -> temp_enh
...
save(enh_prom_links,file = "Data/enh_prom_links_CHIC.RData")
#####################
